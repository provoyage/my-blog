...
export function PostCard({ post, featured = false, preload = false }: PostCardProps) {
  const articleHref = post.slug ? `/articles/${post.slug}` : "/articles";
  const imageSrc = post.image || "/media/categories/gray-hair.jpg";
  const summary = post.excerpt?.trim() || post.description?.trim() || "記事の概要は準備中です。";
  const concernTags =
    post.targetConcern.length > 0 ? post.targetConcern.slice(0, 2) : [post.categoryName];

  return (
    <article className="group glass-panel flex h-full min-w-0 flex-col overflow-hidden rounded-[2rem] transition duration-300 hover:-translate-y-1 hover:shadow-[0_30px_90px_rgba(26,35,45,0.14)]">
      <Link href={articleHref} className="block">
        ...
        <div className={`flex h-full min-w-0 flex-col ${featured ? "p-5 sm:p-8" : "p-5 sm:p-6"}`}>
          ...
          <h3
            className={`mt-4 font-serif tracking-[-0.03em] text-slate-950 transition group-hover:text-slate-700 ${
              featured
                ? "overflow-hidden break-words text-[2rem] leading-[1.08] tracking-[-0.04em] [display:-webkit-box] [-webkit-box-orient:vertical] [-webkit-line-clamp:2] sm:text-[2.2rem]"
                : "break-words text-xl leading-snug sm:text-2xl"
            }`}
          >
            {post.title}
          </h3>
          ...
          <p
            className={`mt-4 text-slate-600 ${
              featured
                ? "overflow-hidden break-words text-sm leading-7 [display:-webkit-box] [-webkit-box-orient:vertical] [-webkit-line-clamp:3] sm:text-base sm:leading-8"
                : "text-sm leading-7"
            }`}
          >
            {summary}
      ...
      <div className={`${featured ? "px-5 pb-5 sm:px-8 sm:pb-8" : "px-5 pb-5 sm:px-6 sm:pb-6"} mt-auto`}>
        <div className="flex flex-col gap-4 border-t border-white/60 pt-5 text-sm text-slate-500 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center">
            {post.affiliateUrl ? (
              <a
                ...
                className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-slate-950 px-4 py-2 text-sm font-medium text-white transition hover:bg-slate-800 sm:w-auto"
              >
                <ExternalLinkIcon />
            ...
            <Link
              href={articleHref}
              className="inline-flex w-full items-center justify-center rounded-full bg-white/85 px-3 py-2 text-slate-700 transition hover:bg-white sm:w-auto sm:py-1"
            >
              記事を読む
            </Link>
          </div>
